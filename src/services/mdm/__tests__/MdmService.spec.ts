...
vi.mock("@superroo/cloud", () => ({
	...
	PRODUCTION_CLERK_BASE_URL: "https://clerk.superroo.com",
}))
	...
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"mdm.errors.cloud_auth_required":
				"Your organization requires SuperRoo Cloud authentication. Please sign in to continue.",
			"mdm.errors.organization_mismatch":
				"You must be authenticated with your organization's SuperRoo Cloud account.",
...
import { CloudService, getClerkBaseUrl, PRODUCTION_CLERK_BASE_URL } from "@superroo/cloud"
...
describe("MdmService", () => {
	...
	beforeEach(() => {
		...
		mockGetClerkBaseUrl.mockReturnValue("https://dev.clerk.superroo.com")
	...
	describe("platform-specific config paths", () => {
		...
		it("should use correct path for Windows in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "SuperRoo", "mdm.json"))
		})
		...
		it("should use correct path for Windows in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "SuperRoo", "mdm.dev.json"))
		})
		...
		it("should use correct path for macOS in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/SuperRoo/mdm.json")
		})
		...
		it("should use correct path for macOS in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/SuperRoo/mdm.dev.json")
		})
		...
		it("should use correct path for Linux in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/superroo/mdm.json")
		})
		...
		it("should use correct path for Linux in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/superroo/mdm.dev.json")
		})
	...
	describe("compliance checking", () => {
		...
		it("should be non-compliant when not authenticated", async () => {
			...
			if (!compliance.compliant) {
				expect(compliance.reason).toContain("Your organization requires SuperRoo Cloud authentication")
			}
		})
