...
export class MdmService {
	...
	private getMdmConfigPath(): string {
		...
		switch (platform) {
			case "win32": {
				// Windows: %ProgramData%\SuperRoo\mdm.json or mdm.dev.json
				const programData = process.env.PROGRAMDATA || "C:\\ProgramData"
				return path.join(programData, "SuperRoo", configFileName)
			}

			case "darwin":
				// macOS: /Library/Application Support/SuperRoo/mdm.json or mdm.dev.json
				return `/Library/Application Support/SuperRoo/${configFileName}`

			case "linux":
			default:
				// Linux: /etc/superroo/mdm.json or mdm.dev.json
				return `/etc/superroo/${configFileName}`
		}
	}
