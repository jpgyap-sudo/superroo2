...
Instructions`)
	...
	describe("autoGenerateSkill", () => {
		it("should auto-generate a workspace skill from project signals", async () => {
			mockDirectoryExists.mockResolvedValue(false)
			mockRealpath.mockImplementation(async (p: string) => p)
			mockReaddir.mockResolvedValue([])
			mockFileExists.mockImplementation(async (file: string) => {
				return file === p(PROJECT_DIR, "pnpm-workspace.yaml") || file === p(PROJECT_DIR, "tsconfig.json")
			})
			mockReadFile.mockImplementation(async (file: string) => {
				if (file === p(PROJECT_DIR, "package.json")) {
					return JSON.stringify({
						name: "superroo-test",
						dependencies: { react: "latest" },
						devDependencies: { vitest: "latest", typescript: "latest", "@types/vscode": "latest" },
					})
				}
				if (file === p(PROJECT_DIR, "AGENTS.md")) {
					return "SettingsView inputs must bind to cachedState."
				}
				throw new Error("not found")
			})
			mockMkdir.mockResolvedValue(undefined)
			...
			const result = await skillsManager.autoGenerateSkill({
				source: "project",
				goal: "learn project coding patterns",
				modeSlugs: ["code"],
			})

			expect(result.path).toBe(p(PROJECT_DIR, ".roo", "skills", "superroo-test-coding-skill", "SKILL.md"))
			expect(result.name).toBe("superroo-test-coding-skill")
			expect(mockWriteFile).toHaveBeenCalled()

			const writeCall = mockWriteFile.mock.calls[0]
			expect(writeCall[1]).toContain("Auto-generated by SuperRoo")
			expect(writeCall[1]).toContain("React UI dependency detected")
			expect(writeCall[1]).toContain("cachedState")
			expect(writeCall[1]).toContain("modeSlugs:")
			expect(writeCall[1]).toContain("- code")
		})

		it("should auto-generate a global skill", async () => {
			mockDirectoryExists.mockResolvedValue(false)
			mockRealpath.mockImplementation(async (p: string) => p)
			mockReaddir.mockResolvedValue([])
			mockFileExists.mockResolvedValue(false)
			...
			const result = await skillsManager.autoGenerateSkill({ source: "global" })

			expect(result.path).toBe(p(GLOBAL_ROO_DIR, "skills", "superroo-neural-coding", "SKILL.md"))
			expect(result.name).toBe("superroo-neural-coding")
			expect(mockWriteFile.mock.calls[0][1]).toContain("ML And Neural Signals")
		})
	})
