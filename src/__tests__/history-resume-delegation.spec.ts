...
import { SuperRooEventName } from "@superroo/types"
...
describe("History resume delegation - parent metadata transitions", () => {
	...
	it("reopenParentFromDelegation emits events in correct order: TaskDelegationCompleted → TaskDelegationResumed", async () => {
		...
		expect(eventNames).toContain(SuperRooEventName.TaskDelegationCompleted)
		expect(eventNames).toContain(SuperRooEventName.TaskDelegationResumed)

		// CRITICAL: verify ordering (TaskDelegationCompleted before TaskDelegationResumed)
		const completedIdx = emitSpy.mock.calls.findIndex((c) => c[0] === SuperRooEventName.TaskDelegationCompleted)
		const resumedIdx = emitSpy.mock.calls.findIndex((c) => c[0] === SuperRooEventName.TaskDelegationResumed)
	...
	it("reopenParentFromDelegation continues when overwrite operations fail and still resumes/emits (RPD-06)", async () => {
		...
		expect(emitSpy).toHaveBeenCalledWith(
			SuperRooEventName.TaskDelegationCompleted,
			"parent-rpd06",
			"child-rpd06",
			"Subtask finished despite overwrite failures",
		)
		expect(emitSpy).toHaveBeenCalledWith(SuperRooEventName.TaskDelegationResumed, "parent-rpd06", "child-rpd06")
	...
	it("reopenParentFromDelegation does NOT emit TaskPaused or TaskUnpaused (new flow only)", async () => {
		...
		const eventNames = emitSpy.mock.calls.map((c) => c[0])
		expect(eventNames).not.toContain(SuperRooEventName.TaskPaused)
		expect(eventNames).not.toContain(SuperRooEventName.TaskUnpaused)
		expect(eventNames).not.toContain(SuperRooEventName.TaskSpawned)
	})
	...
	it("reopenParentFromDelegation logs child status persistence failure and continues reopen flow (RPD-04)", async () => {
		...
		expect(parentInstance.resumeAfterDelegation).toHaveBeenCalledTimes(1)
		expect(emitSpy).toHaveBeenCalledWith(SuperRooEventName.TaskDelegationResumed, "parent-rpd04", "child-rpd04")
	})
