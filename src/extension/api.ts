...
import { IpcServer } from "@superroo/ipc"
...
export class API extends EventEmitter<SuperRooEvents> implements SuperRooAPI {
	...
	constructor(
		...
		if (enableLogging) {
			...
			this.logfile = path.join(os.tmpdir(), "superroo-messages.log")
		} else {
			this.log = () => {}
		}
		...
		if (socketPath) {
			...
			ipc.on(IpcMessageType.TaskCommand, async (clientId, command) => {
				const sendResponse = (eventName: SuperRooEventName, payload: unknown[]) => {
					ipc.send(clientId, {
				...
				switch (command.commandName) {
					...
					case TaskCommandName.GetCommands:
						try {
							...
							sendResponse(SuperRooEventName.CommandsResponse, [
								commands.map((cmd) => ({
									name: cmd.name,
									source: cmd.source,
					...
					case TaskCommandName.GetModes:
						try {
							...
							sendResponse(SuperRooEventName.ModesResponse, [modes])
						} catch (error) {
							sendResponse(SuperRooEventName.ModesResponse, [[]])
						}
					...
					case TaskCommandName.GetModels:
						try {
							const models = await getModels({
								...
								baseUrl: process.env.SUPERROO_PROVIDER_URL ?? "https://api.superroo.com/proxy",
								apiKey: CloudService.hasInstance()
									? CloudService.instance.authService?.getSessionToken()
									: undefined,
							})

							sendResponse(SuperRooEventName.ModelsResponse, [models])
						} catch (error) {
							sendResponse(SuperRooEventName.ModelsResponse, [{}])
						}
	...
	public override emit<K extends keyof SuperRooEvents>(
		eventName: K,
		...args: K extends keyof SuperRooEvents ? SuperRooEvents[K] : never
	) {
		const data = { eventName: eventName as SuperRooEventName, payload: args } as TaskEvent
	...
	private registerListeners(provider: ClineProvider) {
		provider.on(SuperRooEventName.TaskCreated, (task) => {
			// Task Lifecycle

			task.on(SuperRooEventName.TaskStarted, async () => {
				this.emit(SuperRooEventName.TaskStarted, task.taskId)
				await this.fileLog(`[${new Date().toISOString()}] taskStarted -> ${task.taskId}\n`)
			})

			task.on(SuperRooEventName.TaskCompleted, async (_, tokenUsage, toolUsage) => {
				this.emit(SuperRooEventName.TaskCompleted, task.taskId, tokenUsage, toolUsage, {
			...
			task.on(SuperRooEventName.TaskAborted, () => {
				this.emit(SuperRooEventName.TaskAborted, task.taskId)
			})

			task.on(SuperRooEventName.TaskFocused, () => {
				this.emit(SuperRooEventName.TaskFocused, task.taskId)
			})

			task.on(SuperRooEventName.TaskUnfocused, () => {
				this.emit(SuperRooEventName.TaskUnfocused, task.taskId)
			})

			task.on(SuperRooEventName.TaskActive, () => {
				this.emit(SuperRooEventName.TaskActive, task.taskId)
			})

			task.on(SuperRooEventName.TaskInteractive, () => {
				this.emit(SuperRooEventName.TaskInteractive, task.taskId)
			})

			task.on(SuperRooEventName.TaskResumable, () => {
				this.emit(SuperRooEventName.TaskResumable, task.taskId)
			})

			task.on(SuperRooEventName.TaskIdle, () => {
				this.emit(SuperRooEventName.TaskIdle, task.taskId)
			})

			// Subtask Lifecycle

			task.on(SuperRooEventName.TaskPaused, () => {
				this.emit(SuperRooEventName.TaskPaused, task.taskId)
			})

			task.on(SuperRooEventName.TaskUnpaused, () => {
				this.emit(SuperRooEventName.TaskUnpaused, task.taskId)
			})

			task.on(SuperRooEventName.TaskSpawned, (childTaskId) => {
				this.emit(SuperRooEventName.TaskSpawned, task.taskId, childTaskId)
			})

			task.on(SuperRooEventName.TaskDelegated as any, (childTaskId: string) => {
				;(this.emit as any)(SuperRooEventName.TaskDelegated, task.taskId, childTaskId)
			})

			task.on(SuperRooEventName.TaskDelegationCompleted as any, (childTaskId: string, summary: string) => {
				;(this.emit as any)(SuperRooEventName.TaskDelegationCompleted, task.taskId, childTaskId, summary)
			})

			task.on(SuperRooEventName.TaskDelegationResumed as any, (childTaskId: string) => {
				;(this.emit as any)(SuperRooEventName.TaskDelegationResumed, task.taskId, childTaskId)
			})

			// Task Execution

			task.on(SuperRooEventName.Message, async (message) => {
				this.emit(SuperRooEventName.Message, { taskId: task.taskId, ...message })
			...
			task.on(SuperRooEventName.TaskModeSwitched, (taskId, mode) => {
				this.emit(SuperRooEventName.TaskModeSwitched, taskId, mode)
			})

			task.on(SuperRooEventName.TaskAskResponded, () => {
				this.emit(SuperRooEventName.TaskAskResponded, task.taskId)
			})

			task.on(SuperRooEventName.QueuedMessagesUpdated, (taskId, messages) => {
				this.emit(SuperRooEventName.QueuedMessagesUpdated, taskId, messages)
			})

			// Task Analytics

			task.on(SuperRooEventName.TaskToolFailed, (taskId, tool, error) => {
				this.emit(SuperRooEventName.TaskToolFailed, taskId, tool, error)
			})

			task.on(SuperRooEventName.TaskTokenUsageUpdated, (_, tokenUsage, toolUsage) => {
				this.emit(SuperRooEventName.TaskTokenUsageUpdated, task.taskId, tokenUsage, toolUsage)
			})

			// Let's go!

			this.emit(SuperRooEventName.TaskCreated, task.taskId)
		})
	}
	...
	public getConfiguration(): SuperRooSettings {
		return Object.fromEntries(
			Object.entries(this.sidebarProvider.getValues()).filter(([key]) => !isSecretStateKey(key)),
		)
	}

	public async setConfiguration(values: SuperRooSettings) {
