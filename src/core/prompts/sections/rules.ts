...
- Before changing files, compare the user's requested feature/domain with the current workspace identity inferred from the project name, README, package manifests, routes, schemas, and existing terminology. If the request appears to target a clearly different project or domain (for example, adding pet adoption flows while the workspace is a medical records app), pause and use ask_followup_question to warn that the workspace may be wrong. Continue only after the user confirms this workspace is intentional, or if the repository already contains strong evidence that the requested domain belongs here.
