...
type SuperRooSettingsKey = keyof SuperRooSettings
...
export class ContextProxy {
	 ...
	 */
	private sanitizeProviderValues(values: SuperRooSettings): SuperRooSettings {
		...
		for (const key of legacyKeys) {
			if (key in sanitizedValues) {
				...
				sanitizedValues = copy as SuperRooSettings
			}
		}
		...
		if (values.apiProvider !== undefined && !isKnownProvider) {
			...
			return restValues as SuperRooSettings
		}
	...
	public async setValue<K extends SuperRooSettingsKey>(key: K, value: SuperRooSettings[K]) {
	...
	public getValue<K extends SuperRooSettingsKey>(key: K): SuperRooSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key as SecretStateKey) as SuperRooSettings[K])
			: (this.getGlobalState(key as GlobalStateKey) as SuperRooSettings[K])
	}

	public getValues(): SuperRooSettings {
	...
	public async setValues(values: SuperRooSettings) {
		const entries = Object.entries(values) as [SuperRooSettingsKey, unknown][]
