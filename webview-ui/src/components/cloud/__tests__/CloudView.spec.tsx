	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"cloud:signIn": "Connect to SuperRoo Cloud",
				"cloud:cloudBenefitsTitle": "Try SuperRoo Cloud",
				...
				"cloud:visitCloudWebsite": "Visit SuperRoo Cloud",
				"cloud:taskSync": "Task sync",
				"cloud:taskSyncDescription": "Sync your tasks for viewing and sharing on SuperRoo Cloud",
				...
				"cloud:cloudUrlPillLabel": "SuperRoo Cloud URL: ",
			}
...
describe("CloudView", () => {
	it("should display benefits when user is not authenticated", () => {
		render(<CloudView userInfo={null} isAuthenticated={false} cloudApiUrl="https://app.superroo.com" />)

		// Check that the benefits section is displayed
		expect(screen.getByRole("heading", { name: "Try SuperRoo Cloud" })).toBeInTheDocument()
	...
	it("should not display benefits when user is authenticated", () => {
		...
		render(<CloudView userInfo={mockUserInfo} isAuthenticated={true} cloudApiUrl="https://app.superroo.com" />)
	...
	it("should not display cloud URL pill when pointing to production", () => {
		...
		expect(screen.queryByText(/SuperRoo Cloud URL:/)).not.toBeInTheDocument()
	})
	...
	it("should display cloud URL pill when pointing to non-production environment", () => {
		...
		render(<CloudView userInfo={mockUserInfo} isAuthenticated={true} cloudApiUrl="https://staging.superroo.com" />)

		// Check that the cloud URL pill is displayed with the staging URL
		expect(screen.getByText(/SuperRoo Cloud URL:/)).toBeInTheDocument()
		expect(screen.getByText("https://staging.superroo.com")).toBeInTheDocument()
	})

	it("should display cloud URL pill for non-authenticated users when not pointing to production", () => {
		render(<CloudView userInfo={null} isAuthenticated={false} cloudApiUrl="https://dev.superroo.com" />)

		// Check that the cloud URL pill is displayed even when not authenticated
		expect(screen.getByText(/SuperRoo Cloud URL:/)).toBeInTheDocument()
		expect(screen.getByText("https://dev.superroo.com")).toBeInTheDocument()
	})
