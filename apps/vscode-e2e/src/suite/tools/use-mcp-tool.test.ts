...
import { SuperRooEventName, type ClineMessage } from "@superroo/types"

import { waitFor, sleep } from "../utils"
import { setDefaultSuiteTimeout } from "../test-utils"

suite.skip("SuperRoo use_mcp_tool Tool", function () {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(SuperRooEventName.Message, messageHandler)
		...
		api.on(SuperRooEventName.TaskStarted, taskStartedHandler)
		...
		api.on(SuperRooEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for SuperRoo to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this SuperRoo won't load the MCP server
		...
		try {
			...
			// Clean up
			api.off(SuperRooEventName.Message, messageHandler)
			api.off(SuperRooEventName.TaskStarted, taskStartedHandler)
			api.off(SuperRooEventName.TaskCompleted, taskCompletedHandler)
		}
	})
