...
suite.skip("SuperRoo Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, ClineMessage[]> = {}

		api.on(SuperRooEventName.Message, ({ taskId, message }) => {
		...
		api.on(SuperRooEventName.TaskSpawned, (_, childTaskId) => (spawnedTaskId = childTaskId))
