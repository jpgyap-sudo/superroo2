...
import { type TokenUsage, type ToolUsage, SuperRooEventName, taskEventSchema } from "@superroo/types"
import type { Run, Task, TaskMetrics } from "@superroo/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case SuperRooEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case SuperRooEventName.TaskTokenUsageUpdated: {
			...
			case SuperRooEventName.EvalPass:
			case SuperRooEventName.EvalFail:
