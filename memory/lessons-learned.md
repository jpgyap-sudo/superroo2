...
### Auto-Extracted Lesson: Fix skills frontmatter, ripgrep universal path, dashboard Windows build, remo...

Date: 2026-05-30
Source: Git commit f737e7a0
Model/API used: unknown
Confidence: medium
Related files: .roo/skills/commissioning-agent/SKILL.md, .roo/skills/docker-upgrade/SKILL.md, .roo/skills/terminal-brain-upgrade/SKILL.md, cloud/dashboard/next.config.js, cloud/dashboard/scripts/build-safe.mjs

#### Task Summary

fix: fix skills frontmatter, ripgrep universal path, dashboard Windows build, remove react-compiler

#### Files Changed

- `.roo/skills/commissioning-agent/SKILL.md`
- `.roo/skills/docker-upgrade/SKILL.md`
...
Unknown — extracted from commit f737e7a0.

#### Fix Applied

<!-- TODO: Document the solution -->

See commit f737e7a0 by superroo.
...
### Lesson: Thinker Agent Workflow — Four-layer thinking process for Kilo Code

Date: 2026-05-31
Source: Kilo Code task completion
Model/API used: kilo-auto/free
Confidence: high
Related files: .kilo/agent/thinker.md, .kilo/agent/architect.md, .kilo/agent/coder.md, .kilo/agent/reviewer.md, .kilo/command/think-and-plan.md, scripts/central-brain-mcp.mjs, .mcp.json, .kilo/workflow.md

#### Task Summary

Created a four-layer thinking process for Kilo Code: (1) Think and plan using Auto Free for model routing, (2) Read AGENTS.md and project configuration for context, (3) Delegate to architect/coder agents, (4) Review by reviewer agent. Added PostgreSQL/pgvector integration to Central Brain MCP server with JSONL fallback.

#### Files Changed

- .kilo/agent/thinker.md — created thinker agent with four-step workflow
- .kilo/agent/architect.md — updated to receive pre-analyzed tasks from thinker
- .kilo/agent/coder.md — updated to receive structured plans from architect
- .kilo/agent/reviewer.md — created reviewer agent for code review
- .kilo/command/think-and-plan.md — created command documentation
- scripts/central-brain-mcp.mjs — added PostgreSQL/pgvector integration with JSONL fallback
- .mcp.json — removed deepseek-coder, added central-brain with DATABASE_URL
- .kilo/workflow.md — documented complete workflow
...
Created thinker, architect, coder, and reviewer agents with explicit workflow steps. Updated Central Brain MCP to connect to PostgreSQL/pgvector with JSONL fallback. Removed DeepSeek dependency, using Ollama models exclusively.

#### Test Result

pass — agent configurations validated

#### Lesson Learned

Four-layer agent workflows (thinker → architect → coder → reviewer) provide better separation of concerns. PostgreSQL/pgvector integration enables persistent memory with semantic search. JSONL fallback ensures the system works even when the database is unavailable.

#### Reusable Rule

When building multi-agent systems, use explicit workflow layers with clear handoffs. Always implement database fallback to local storage for resilience.

#### Tags

workflow, thinker-agent, architect-agent, coder-agent, reviewer-agent, central-brain, mcp, postgres, pgvector, kilo-code

---

### Lesson: Reviewer Agent — Code review and quality assurance

Date: 2026-05-31
Source: Kilo Code task completion
Model/API used: kilo-auto/free
Confidence: high
Related files: .kilo/agent/reviewer.md

#### Task Summary

Created reviewer agent using Auto Free for code review and quality assurance. The reviewer is the final step in the four-layer workflow, ensuring code quality before completion.

#### Files Changed

- .kilo/agent/reviewer.md — created reviewer agent configuration
...
Created reviewer agent with structured review output format and lesson obligation integration.

#### Test Result

pass — agent configuration validated

#### Lesson Learned

A dedicated reviewer agent improves code quality by providing systematic review for bugs, security, performance, and best practices.

#### Reusable Rule

Every implementation workflow should include a review step with structured feedback and lesson contribution.

#### Tags

reviewer-agent, code-review, quality-assurance, kilo-code

---

### Lesson: Shell Keyword Detection Tightening

Date: 2026-05-17
Source: Codex/Kimi legacy session
Model/API used: claude-sonnet-4-6
Confidence: medium
Related files: cloud/api/telegramPolicy.js, cloud/api/telegramClassifier.js

#### Task Summary

Shell Keyword Detection Tightening

#### Lesson Learned

VPS version queries misclassified as unsafe shell intent, triggering policy block instead of answering

#### Reusable Rule

Added inferShellCommand() pre-classifier and relaxed canRunWithoutApproval to only block blockedActions

#### Tags

telegram, safety, nlp, shell, policy

---

### Lesson: SuperRoo uses three parallel learning layer stores
...
Engineering lesson from Claude Code session.

#### Lesson Learned

The SuperRoo learning layer maintains three synchronized stores: (1) memory/lessons-learned.md — human-readable markdown, (2) memory/lesson-index.jsonl — structured JSON index for getLessonRetriever(), (3) memory/lesson-summaries.json — AI summaries with embedding metadata. All three must be updated together.

#### Reusable Rule

The SuperRoo learning layer maintains three synchronized stores: (1) memory/lessons-learned.md — human-readable markdown, (2) memory/lesson-index.jsonl — structured JSON index for getLessonRetriever(), (3) memory/lesson-summaries.json — AI summaries with embedding metadata.

#### Tags

learning-layer, architecture, knowledge-management

---

### Lesson: SKILL.md YAML frontmatter: always use --- delimiters and quote colons
...
Bug fix from Claude Code session.

#### Lesson Learned

Three .roo/skills/\*/SKILL.md files had missing --- delimiters or unquoted colons in description values. YAML treats unquoted colons as key-value pairs, breaking the frontmatter. Fix: add --- at top and bottom, wrap any description with : in double quotes.

#### Reusable Rule

Three .roo/skills/\*/SKILL.md files had missing --- delimiters or unquoted colons in description values.

#### Tags

yaml, skills, vscode-extension, frontmatter, startup

---

### Lesson: ripgrep resolution: add @vscode/ripgrep-universal/bin/[platform]-[arch]/ to getBinPath
...
Modern VS Code ships ripgrep via @vscode/ripgrep-universal with platform-specific subdirectory (e.g. win32-x64/rg.exe). The extension's getBinPath() waterfall must include this path. Derive platform as process.platform + '-' + process.arch and append to the waterfall in src/services/ripgrep/index.ts.

#### Reusable Rule

Modern VS Code ships ripgrep via @vscode/ripgrep-universal with platform-specific subdirectory (e.g.

#### Tags

ripgrep, vscode-extension, binary-resolution, cross-platform

---

### Lesson: Next.js standalone output: gate on process.platform !== win32 for symlinks
...
Next.js output:standalone creates symlinks in .next/standalone/node_modules. On Windows, fs.symlink() throws EPERM unless the process has SeCreateSymbolicLinkPrivilege. Fix: check process.platform === 'win32' in next.config.js and build-safe.mjs to skip standalone mode on Windows.

#### Reusable Rule

Next.js output:standalone creates symlinks in .next/standalone/node_modules.

#### Tags

nextjs, windows, symlink, EPERM, build, standalone

---

### Lesson: Remove babel-plugin-react-compiler — breaks VS Code webview builds
...
babel-plugin-react-compiler is experimental. In complex VS Code webview apps with extension-state bridges, its auto-memoization transforms break component identity. Remove from webview-ui/vite.config.ts: replace react({ babel: { plugins: [...compiler...] } }) with plain react().

#### Reusable Rule

babel-plugin-react-compiler is experimental.

#### Tags

react-compiler, babel, vite, webview, experimental, build

---

### Lesson: Ollama: always try local 127.0.0.1:11434 before VPS 100.64.175.88:11434
...
Pattern observed from Claude Code session.

#### Lesson Learned

Any script using Ollama should implement local-first resolution: GET http://127.0.0.1:11434/api/tags with 1-2s timeout first, fall back to VPS (100.64.175.88:11434) if local unavailable. Log which endpoint was selected. Hardcoding the VPS URL adds unnecessary Tailscale dependency.

#### Reusable Rule

Any script using Ollama should implement local-first resolution: GET http://127.0.0.1:11434/api/tags with 1-2s timeout first, fall back to VPS (100.64.175.88:11434) if local unavailable.

#### Tags

ollama, local-first, tailscale, endpoint-resolution, pattern

---

### Lesson: VS Code webview blank panel: wrap HTML generation in try/catch with logging
...
VS Code webview failures are silent by default — an exception in resolveWebviewView shows only a blank panel. Fix: wrap getHtmlContent()/getHMRHtmlContent() calls in try/catch. On success log HTML length. On failure log error.message + stack. Also log scriptUri/stylesUri/cspSource before generation.

#### Reusable Rule

VS Code webview failures are silent by default — an exception in resolveWebviewView shows only a blank panel.

#### Tags

vscode-extension, webview, debugging, blank-panel, error-handling

---

### Lesson: MermaidBlock: fall back to CodeBlock when mermaid module breaks on version update
...
Mermaid v11.4.1 restructured its module paths. The 331-line MermaidBlock component broke with module-not-found errors. Simplified to 13 lines delegating to CodeBlock. This unblocks the build. The complex renderer can be restored after the upstream module structure is understood.

#### Reusable Rule

Mermaid v11.4.1 restructured its module paths.

#### Tags

mermaid, webview, build, fallback, simplification

---

### Lesson: DeepSeek V4 model names: deepseek-v4-flash (no -chat- infix)

Date: 2026-05-31
Source: Claude Code brain (claude-brain/knowledge.jsonl)
Model/API used: claude-sonnet-4-6
Confidence: high
Related files: cloud/providers/deepseek.js, cloud/api/api.js
...
DeepSeek V4 dropped the -chat- infix. Old: deepseek-chat-v4-flash, deepseek-chat-v4-pro. New: deepseek-v4-flash, deepseek-v4-pro. API returns 404 for old names. All model names must be defined as named constants in cloud/providers/deepseek.js and imported everywhere else.

#### Reusable Rule

DeepSeek V4 dropped the -chat- infix.

#### Tags

deepseek, model-names, api, constants, provider

---

### Lesson: Telegram bot: missing brace in if(orchestratorBridge) swallows all reply logic
...
A missing closing brace for if(orchestratorBridge) in telegramBot.js caused all Telegram reply logic (sendMessage, error handlers) to be inside that condition. When orchestratorBridge was null, bot sent no responses. Fix: always run Prettier after large refactors to catch brace mismatches.

#### Reusable Rule

A missing closing brace for if(orchestratorBridge) in telegramBot.js caused all Telegram reply logic (sendMessage, error handlers) to be inside that condition.

#### Tags

telegram, javascript, brace-mismatch, silent-failure, debugging

---

### Lesson: Telegram routing: all handler paths must go through orchestratorBridge.submitDirect()

Date: 2026-05-31
Source: Claude Code brain (claude-brain/knowledge.jsonl)
Model/API used: claude-sonnet-4-6
Confidence: high
Related files: cloud/api/telegramBot.js, cloud/orchestrator/TelegramOrchestratorBridge.js
...
The Telegram bot has multiple intent paths (code, feature, bug, chat, fallback). All must route through TelegramOrchestratorBridge.submitDirect() for queue management, rate limiting, and learning layer hooks. Direct API calls bypass all of these. submitDirect() is the single entry point for all task types.

#### Reusable Rule

The Telegram bot has multiple intent paths (code, feature, bug, chat, fallback).

#### Tags

telegram, orchestrator, routing, architecture, pattern

---

### Lesson: node-pty + xterm.js: only correct approach for web-based interactive terminals
...
exec/spawn without PTY cannot handle interactive programs (vi, python REPL, ssh). Web terminals need: (1) node-pty on the server for a real PTY process, (2) WebSocket for bidirectional I/O, (3) xterm.js on the frontend for ANSI rendering + FitAddon for resize. All three are required.

#### Reusable Rule

exec/spawn without PTY cannot handle interactive programs (vi, python REPL, ssh).

#### Tags

node-pty, xterm, terminal, websocket, pty, interactive

---

### Lesson: API polling: always check response.ok and wrap JSON.parse in try/catch
...
API polling components crash when the endpoint returns an HTML error page (nginx 502/503 during restarts). Required pattern: (1) check response.ok before parsing, (2) wrap JSON.parse in try/catch, (3) show 'reconnecting' UI state on failure, never an unhandled crash.

#### Reusable Rule

API polling components crash when the endpoint returns an HTML error page (nginx 502/503 during restarts).

#### Tags

api-polling, json-parse, error-handling, react, nginx, rule

---

### Lesson: HermesClaw already integrates with askAI for context-enriched answers
...
The askAI() function in telegramBot.js (line 2749-2765) already calls orchestrator.hermesClaw.recallContext() to inject relevant past experiences into the system prompt before calling the AI. The /ask command uses this. The new /hermes ask subcommand extends this with Claude-API-specific routing.

#### Reusable Rule

The askAI() function in telegramBot.js (line 2749-2765) already calls orchestrator.hermesClaw.recallContext() to inject relevant past experiences into the system prompt before calling the AI.

#### Tags

hermes, askAI, rag, telegram, architecture

---

### Lesson: AGENTS.md: agent-specific workflows supersede MCP, but learning layer is mandatory
...
Clarification added to AGENTS.md: agents may use their preferred coding workflows (Claude Code using its own tools, DeepSeek using MCP, etc.) but MUST contribute to the shared learning layer (wf-004). The Central Brain is the master source of truth. MCP orchestration is the default, not a hard requirement.

#### Reusable Rule

Clarification added to AGENTS.md: agents may use their preferred coding workflows (Claude Code using its own tools, DeepSeek using MCP, etc.) but MUST contribute to the shared learning layer (wf-004).

#### Tags

agents, multi-agent, workflow, learning-layer, architecture, decision

---

### Lesson: Ollama Docker: explicit perf env vars make VPS inference dramatically faster
...
Ollama defaults are conservative. On a 16+ core VPS with large RAM, set: OLLAMA_NUM_THREADS=16, OLLAMA_MAX_LOADED_MODELS=5, OLLAMA_CONTEXT_LENGTH=65536, OLLAMA_FLASH_ATTENTION=1, OLLAMA_KV_CACHE_QUANTIZATION=8bit, OLLAMA_BATCH_SIZE=512, OLLAMA_KEEP_ALIVE=1h. Use shell-default syntax so docker-compose can override.

#### Reusable Rule

Ollama defaults are conservative.

#### Tags

ollama, docker, performance, vps, env-vars

---

### Lesson: nginx proxy: trailing slash on proxy_pass strips the location prefix
...
nginx location /prefix/ { proxy_pass http://backend; } forwards the full path including /prefix/. To strip the prefix so the backend sees paths without /prefix/, use a trailing slash: proxy_pass http://backend/. This is required for mini-IDE (/tg/ prefix) and any backend that doesn't expect the routing prefix.

#### Reusable Rule

nginx location /prefix/ { proxy_pass http://backend; } forwards the full path including /prefix/.

#### Tags

nginx, proxy, path-stripping, routing, rule

---

### Lesson: CI/CD pipeline: never skip tests on main branch, always gate deploys on green

Date: 2026-05-31
Source: Claude Code observation
Model/API used: claude-sonnet-4-6
Confidence: high
Related files: .github/workflows/, cloud/remote-deploy-dashboard.sh

#### Task Summary

CI/CD pipeline: never skip tests on main branch, always gate deploys on green

#### Lesson Learned

SuperRoo uses PM2 on VPS with no automated CI gate between commit and deploy. Deploys have failed due to TypeScript errors and missing dependencies that a CI check would have caught. A green CI run is the minimum bar before any production deploy.

#### Reusable Rule

Never deploy to production without a passing CI gate. All commits to main must pass lint, type-check, and unit tests before the deploy step runs.

#### Tags

ci-cd, testing, deployment, github-actions

---

### Lesson: Caching strategy: cache AI embeddings and model outputs to avoid redundant Ollama calls

Date: 2026-05-31
Source: Claude Code observation
Model/API used: claude-sonnet-4-6
Confidence: high
Related files: cloud/orchestrator/stores/BugKnowledgeStore.js, cloud/orchestrator/stores/adapters/PgVectorAdapter.js

#### Task Summary

Caching strategy: cache AI embeddings and model outputs to avoid redundant Ollama calls

#### Lesson Learned

Ollama embedding calls (nomic-embed-text) take 100-300ms each. Generating embeddings for 360+ lessons on every sync is expensive. Embeddings are deterministic — same input always produces same output — so caching by content hash in the DB eliminates redundant Ollama calls entirely.

#### Reusable Rule

Cache embeddings in PostgreSQL after first generation — never re-embed the same text twice. Cache Hermes 3 summarizations by content hash for lesson summaries.

#### Tags

caching, ollama, embeddings, performance, pgvector

---

### Lesson: PostgreSQL pgvector: always create HNSW index on embedding column for sub-10ms similarity search

Date: 2026-05-31
Source: Claude Code observation
Model/API used: claude-sonnet-4-6
Confidence: high
Related files: cloud/orchestrator/stores/brain/schema.sql, cloud/orchestrator/stores/adapters/PgVectorAdapter.js

#### Task Summary

PostgreSQL pgvector: always create HNSW index on embedding column for sub-10ms similarity search
...
Always create an HNSW index (CREATE INDEX ON table USING hnsw (embedding vector_cosine_ops)) on any pgvector embedding column used for similarity search — without it, every query does a full sequential scan.

#### Tags

postgresql, pgvector, performance, database, embeddings, hnsw
