...
import type { ToolUsage } from "@superroo/types"
...
export const resetEvalsRepo = async ({ run, cwd }: { run: Run; cwd: string }) => {
	await execa({ cwd })`git config user.name "SuperRoo"`
	await execa({ cwd })`git config user.email "support@superroo.com"`
...
export async function copyConversationHistory({
	...
	const extensionStoragePath = "/roo/.vscode/User/globalStorage/rooveterinaryinc.superroo"
