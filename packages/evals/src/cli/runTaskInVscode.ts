...
import { IpcClient } from "@superroo/ipc"
...
export const runTaskInVscode = async ({ run, task, publish, logger, jobToken }: RunTaskOptions) => {
	...
	const env = { SUPERROO_IPC_SOCKET_PATH: ipcSocketPath }
	...
	if (jobToken) {
		codeCommand = `SUPERROO_CLOUD_TOKEN=${jobToken} ${codeCommand}`
	}
	...
	const ignoreEvents: Record<"broadcast" | "log", SuperRooEventName[]> = {
		broadcast: [SuperRooEventName.Message],
		log: [SuperRooEventName.TaskTokenUsageUpdated, SuperRooEventName.TaskAskResponded],
	}
	...
	client.on(IpcMessageType.TaskEvent, async (taskEvent) => {
		...
		if (
			eventName === SuperRooEventName.Message &&
		...
		if (
			...
			if (eventName === SuperRooEventName.Message) {
			...
			if (eventName === SuperRooEventName.Message && payload[0]?.message?.ask === "tool") {
				try {
			...
			} else if (eventName === SuperRooEventName.Message && payload[0]?.message?.ask === "command") {
				logEventName = `${eventName} (command)`
			} else if (eventName === SuperRooEventName.Message && payload[0]?.message?.ask === "completion_result") {
				logEventName = `${eventName} (completion_result)`
			}
			logger.info(`${logEventName} ->`, payload)
		}

		if (eventName === SuperRooEventName.TaskStarted) {
		...
		if (eventName === SuperRooEventName.TaskToolFailed) {
			const [_taskId, toolName, error] = payload
			await createToolError({ taskId: task.id, toolName, error })
		}

		if (eventName === SuperRooEventName.TaskTokenUsageUpdated || eventName === SuperRooEventName.TaskCompleted) {
		...
		if (eventName === SuperRooEventName.TaskAborted) {
			taskAbortedAt = Date.now()
		}

		if (eventName === SuperRooEventName.TaskCompleted) {
