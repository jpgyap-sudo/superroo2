...
import type { SettingsService, AuthService } from "@superroo/types"
...
vi.mock("../Config", () => ({
	getSuperRooApiUrl: () => "https://app.superroo.com",
}))

vi.mock("../utils", () => ({
	getUserAgent: () => "SuperRoo 1.0.0",
}))
	...
	describe("shareTask", () => {
		it("should share task with organization visibility and copy to clipboard", async () => {
			const mockResponseData = {
				success: true,
				shareUrl: "https://app.superroo.com/share/abc123",
			}
			...
			expect(result.shareUrl).toBe("https://app.superroo.com/share/abc123")

			expect(mockFetch).toHaveBeenCalledWith("https://app.superroo.com/api/extension/share", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: "Bearer session-token",
					"User-Agent": "SuperRoo 1.0.0",
				},
				body: JSON.stringify({
					taskId: "task-123",
			...
			expect(vscode.env.clipboard.writeText).toHaveBeenCalledWith("https://app.superroo.com/share/abc123")
		})
