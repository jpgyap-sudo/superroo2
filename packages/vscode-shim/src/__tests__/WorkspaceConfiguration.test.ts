...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("superroo", context)
			...
			setRuntimeConfig("superroo", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("superroo", "testSetting", "testValue")

			expect(getRuntimeConfig("superroo.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("superroo", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("superroo.setting1")).toBe("value1")
			expect(getRuntimeConfig("superroo.setting2")).toBe(42)
			expect(getRuntimeConfig("superroo.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("superroo.defined")).toBe("value")
			expect(getRuntimeConfig("superroo.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("superroo", "setting1", "value1")
			setRuntimeConfig("superroo", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("superroo.setting1")).toBeUndefined()
			expect(getRuntimeConfig("superroo.setting2")).toBeUndefined()
		})
