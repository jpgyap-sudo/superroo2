		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
				...
				<div className="space-y-6">
					...
					<div className="space-y-4 p-5"> 						{(() => { 							const active = tasks.find(t => t.status === "review" || t.status === "waiting_approval" || t.status === "coding") 							if (!active) return <p className="text-sm text-slate-500">No active tasks</p> 							return ( 								<> 								<div className="rounded-2xl border border-[#1e2535] bg-[#0f1117]/60 p-4"> 								<p className="text-xs text-slate-500">Task</p> 								<p className="mt-1 font-semibold text-slate-100">{active.id} · {active.instruction.slice(0, 40)}{active.instruction.length > 40 ? "…" : ""}</p> 								<p className="mt-2 font-mono text-xs text-cyan-300">branch: {active.branchName}</p> 								</div> 								{(active.status === "review" || active.status === "waiting_approval") && ( 								<div className="grid grid-cols-2 gap-3"> 								<button onClick={() => void handleApprove(active.id)} disabled={actionLoading[`approve-${active.id}`]} className="rounded-xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-slate-950 hover:bg-emerald-400 disabled:opacity-50"> 								{actionLoading[`approve-${active.id}`] ? "…" : "Approve"} 								</button> 								<button onClick={() => void handleReject(active.id)} disabled={actionLoading[`reject-${active.id}`]} className="rounded-xl border border-red-500/40 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-300 hover:bg-red-500/20 disabled:opacity-50"> 								{actionLoading[`reject-${active.id}`] ? "…" : "Reject"} 								</button> 								</div> 								)} 								</> 							) 						})() 						} 					</div>
						<div className="space-y-4 p-5">
