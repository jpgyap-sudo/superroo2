...
export default class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
	...
	componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
		console.error("[IdeTerminal ErrorBoundary]", error, errorInfo)
	}
	...
	render() {
		if (this.state.hasError) {
			...
			return (
				<div className="flex flex-col items-center justify-center h-full bg-[#0a0d14] p-6">
					...
					<p className="text-[11px] text-[#8b949e] text-center mb-3 max-w-[300px]">
						{this.state.error?.message || "An unexpected error occurred in the IDE Terminal"}
					</p>
					<button
